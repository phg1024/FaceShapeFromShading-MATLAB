function face_seg(path, options)
enable_visualization = false;
close all;

if nargin > 1
  use_hair_seg = options.use_hair_seg;
else
  use_hair_seg = false;
end
...
parfor i=1:length(all_images)
    ...
    try
        ...
        if true
            hair_mask = hair_seg(I, Ia, points);

            %good_pts = goot_pts .* (1 - hair_mask);
        end
        ...
        for j=1:5
            ...
            if false
                ...
                if use_hair_seg
                  valid_pts = good_pa .* good_p .* good_pts .* (1 - hair_mask);
                else
    ...
    imwrite(hair_mask, fullfile(path, 'masked', ['hair_mask', basename, '.png']));
