...
all_diffs = load(fullfile(path, ['iteration_', num2str(iteration_index)], 'SFS', 'deformation.mat'));
...
for i=1:length(all_images)
    ...
    all_basenames(i) = str2num(basename);
    
    input_image_filename = fullfile(path, all_images{i});    
    input_image{i} = imread(input_image_filename);
    albedo_image_filename = fullfile(path, ['iteration_', num2str(iteration_index)], 'SFS', sprintf('albedo_transferred_%d.png', i-1));
    deformation_image_filename = fullfile(path, ['iteration_', num2str(iteration_index)], 'SFS', sprintf('deformation_%d.png', i-1));
    ...
    deformation_mask_filename = fullfile(path, ['iteration_', num2str(iteration_index)], 'SFS', sprintf('deformation_mask_%d.png', i-1));
    deformation_mask = im2double(imread(deformation_mask_filename));
    
    d = load_depth_map(fullfile(path, ['iteration_', num2str(iteration_index)], 'SFS', sprintf('depth_map%d.bin', i-1)));
    do = load_depth_map(fullfile(path, ['iteration_', num2str(iteration_index)], 'SFS', sprintf('optimized_depth_map_%d.bin', i-1)));
    ...
    max_diff(i) = max(max(all_diffs{i}));
    mean_diff(i) = sum(sum(all_diffs{i})) / sfs_valid_cnt;
...
filtered_images = sorted_idx(1:0.8*length(sorted_idx));
...
filtered_images_indices = sort(all_basenames(filtered_images));
write_indices(fullfile(path, ['iteration_', num2str(iteration_index)], 'SFS', 'selection.txt'), filtered_images_indices);
